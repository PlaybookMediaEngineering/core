...
type TeamORM struct {
	Admin        *UserProfileORM `gorm:"foreignkey:AdminTeamId;association_foreignkey:Id;preload:true"`
	AuditLogs    []*AuditLogORM  `gorm:"foreignkey:TeamId;association_foreignkey:Id;preload:true"`
	CreatedAt    string
	Description  string
	FocusAreas   pq.StringArray `gorm:"type:text[]"`
	Id           uint64
	Members      []*UserProfileORM `gorm:"foreignkey:MembersTeamId;association_foreignkey:Id;preload:true"`
	Publications []*PublicationORM `gorm:"foreignkey:TeamId;association_foreignkey:Id"`
	Roles        []*RoleORM        `gorm:"foreignkey:TeamId;association_foreignkey:Id"`
	Tags         pq.StringArray    `gorm:"type:text[]"`
	TeamName     string
}
...
func DefaultStrictUpdateTeam(ctx context.Context, in *Team, db *gorm.DB) (*Team, error) {
	...
	filterAdmin.AdminTeamId = new(uint64)
	*filterAdmin.AdminTeamId = ormObj.Id
	if err = db.Where(filterAdmin).Delete(UserProfileORM{}).Error; err != nil {
		return nil, err
	}
	...
	filterAuditLogs.TeamId = new(uint64)
	*filterAuditLogs.TeamId = ormObj.Id
	...
	filterMembers.MembersTeamId = new(uint64)
	*filterMembers.MembersTeamId = ormObj.Id
	...
	filterPublications.TeamId = new(uint64)
	*filterPublications.TeamId = ormObj.Id
	if err = db.Where(filterPublications).Delete(PublicationORM{}).Error; err != nil {
		return nil, err
	}
	filterRoles := RoleORM{}
